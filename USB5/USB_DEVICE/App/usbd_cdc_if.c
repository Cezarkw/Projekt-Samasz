...
extern int calculate_CheckSum(char * tab, int n);
extern void zeruj2(char * tab, int n);
extern int rozmiar(char * tab, int n);
extern void zeruj(void);
uint8_t CDC_IsFreeToTransmit(void);
...
//  HAL_StatusTypeDef OK;
  ...
  strlcpy((char*) BufRx, (char *) Buf, (*Len) + 1);
	...
	if (i2c_f==1 && BufRx[0]!='$')
			...
			if (BufRx[0]!=1)
					{
					OK=HAL_I2C_Mem_Write(&hi2c1, Buf[0]<<1, Buf[1], I2C_MEMADD_SIZE_8BIT, &Buf[2], (*Len)-2, HAL_MAX_DELAY);
					if (OK!=HAL_OK)
							CDC_Transmit_FS((uint8_t*)"!ok",3);
					}
				else
					{
					OK=HAL_I2C_Mem_Read(&hi2c1, BufRx[1]<<1, BufRx[2], I2C_MEMADD_SIZE_8BIT, &value, 0x1, HAL_MAX_DELAY);
					if (OK==HAL_OK)
